...
const clickable_slash_commands = [
  {
    "id": "1129687099893878847",
    "name": "help",
    "description": "Displays this help information",
    "options": { "isPublic": true }
  },
  {
    "id": "1129687099893878848",
    "name": "register",
    "description": "Use the current Discord account to register an ADNF account",
    "options": { "isPublic": true }
  },
  {
    "id": "1129687099893878849",
    "name": "link",
    "description": "Link your Discord account to ADNF",
    "options": { "isPublic": true }
  },
  {
    "id": "1129687099893878850",
    "name": "relink",
    "description": "Change the Discord account linked to ADNF",
    "options": { "isPublic": true }
  },
  {
    "id": "1129687099893878851",
    "name": "mention",
    "description": "[ADMIN] It helps locating a player.",
...
const sendPrivate = (pending_content, res) => {
  //  console.log(res, "\n\n", "[LOG] Private response: ", pending_content);
  res.send({
    type: InteractionResponseType.CHANNEL_MESSAGE_WITH_SOURCE,
    data: {
      content: pending_content,
...
const hasAdminRole = async (req, res) => {
  try {
    ...
    const rolesResponse = await discordApi.get(`/guilds/${guildId}/roles`); // get server roles, access
    // discord.com/api/guilds/serverid/roles
...
const handleHelpCommand = async (req, res) => { //~~~~~~~!!!!!!!!!!!!!!!
  let isAdmin = await hasAdminRole(req, res); //!!!!await!!!! wait for the request to finish,.
  // return caught, continue... 
  console.log(parseCommands(clickable_slash_commands, isAdmin)); // log to screen
  sendPrivate(parseCommands(clickable_slash_commands, isAdmin), res); // send to user
};
...
const handleLinkCommand = (req, res) => {
  ...
  const userId = req.body.member.user.id;
  const discriminator = req.body.member.user.discriminator;
  // not implemented, no local network io, should be working with requests delayed
